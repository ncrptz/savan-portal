...
import { BookOpen, Plus, Trash2, Pencil, Check, Radio } from 'lucide-react'
...
export default function AdminVirtualPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2 mb-1"><BookOpen className="w-6 h-6 text-[#000066]" />Virtual Training</h1>
      <p className="text-sm text-gray-500 mb-4">Self-paced theory courses and live lectures.</p>

      <div className="flex gap-2 mb-6">
        <button onClick={() => setTab('courses')} className={`px-3.5 py-1.5 rounded-full text-sm inline-flex items-center gap-1.5 ${tab === 'courses' ? 'bg-[#000066] text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'}`}><BookOpen className="w-4 h-4" />Courses</button>
        <button onClick={() => setTab('live')} className={`px-3.5 py-1.5 rounded-full text-sm inline-flex items-center gap-1.5 ${tab === 'live' ? 'bg-[#000066] text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'}`}><Radio className="w-4 h-4" />Live sessions</button>
      </div>

      {tab === 'live' ? <LiveSessionsAdmin /> : (<>
